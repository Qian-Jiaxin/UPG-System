...
int32_t yt8512c_init(yt8512c_object_t *pobj)
{
    ...
    if (status == YT8512C_STATUS_OK)
    {
        ...
        // while ((HAL_GetTick() - tickstart) <= YT8512C_INIT_TO)
        {
        }
    ...
    if (pobj->io.readreg(pobj->devaddr, YT8512C_BCR, &readval) >= 0)
    {
        ...
        if (pobj->io.writereg(pobj->devaddr, YT8512C_BCR, readval) < 0)
        {
            status =  YT8512C_STATUS_WRITE_ERROR;
				...
				/* 等待自动协商完成 */
				tstart = pobj->io.gettick();
				while ( pobj->io.gettick() - tstart < 100);
				do
				{
...
//    }
//    else if (((readval & YT8512C_SPEED_STATUS) == YT8512C_SPEED_STATUS) && ((readval & YT8512C_DUPLEX_STATUS) != 0))
//    {
...
//    }
		...
		if ((readval & YT8512C_SPEED_STATUS) == YT8512C_SPEED_STATUS)
		{
			if (((readval & YT8512C_DUPLEX_STATUS) != 0))
			{
				return YT8512C_STATUS_100MBITS_FULLDUPLEX;
			}
			else
